...
"""Validation helpers for Layer 8 payloads."""
...
def validate(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    expected_hash = sha3_512_hex(json_dumps({k: v for k, v in payload.items() if k != "hash"}))
    if payload.get("hash") != expected_hash:
        raise SchemaValidationError(
            f"Layer8 hash mismatch: {payload.get('hash')} != {expected_hash}"
        )
    return payload
