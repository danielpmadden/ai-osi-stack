...
│   ├── scripts/ (canonical) – dashboard validation and tooling utilities
...
│   ├── dashboard/ (review) – dashboard planning artefacts duplicating /dashboard/docs/
...
├── scripts/ (canonical) – automation scripts for audits and document generation
...
- `docs/dashboard/` vs `dashboard/docs/`: duplicated dashboard planning notes; converge into a single documentation path under `/docs/dashboard/`.
...
- `docs/public-keys.json`: relocate alongside protocol artefacts under `/protocol/` to align with transport/key distribution guidance.
- `docs/zenodo-metadata.yaml`: move to `/meta/` or `/versions/` with other release metadata manifests.
- `ops/makefile`: relocate into `/tools/` or `/scripts/` to centralize automation entrypoints.
...
- `dashboard/docs/`: fold into `/docs/dashboard/` to avoid split ownership of product documentation.
...
- `dashboard/.husky/` hooks: generated by frontend tooling; guard against committing tool-generated binaries and document regeneration steps.

## Section 5 — Recommendations Summary
- [ ] Delete `/audit/` folder after migrating contents into `/audits/`.
- [ ] Merge dashboard documentation into `/docs/dashboard/` and remove `/dashboard/docs/`.
- [ ] Relocate schema JSON assets (`docs/dashboard/data-model-schema.json`) into a new `/schemas/dashboard/` directory.
- [ ] Move release metadata files (`docs/zenodo-metadata.yaml`, `docs/public-keys.json`) into `/meta/` or `/protocol/` as appropriate.
...
- [ ] Relocate `ops/makefile` and supporting automation into `/scripts/` or `/tools/` to centralize entrypoints.
